...
import { DAY, PRODUCTS, STAGES, STAGE_ORDER, REASONS, toDate, iso, addDays, uid, zeros, nulls, clampInt, seedState, migrate, includedStages, project, atpInfo, atp } from "./engine.js";
...
export default function SupplyDemandPlanner() {
  ...
  const checklist = state.draft ? [
    ...
    { ok: !PRODUCTS.some((p) => state.actuals[p].slice(Math.max(0, nowIdx - 4), nowIdx).some((v) => v == null)) || nowIdx === 0, label: "Plan-vs-actual, last 4 weeks", detail: PRODUCTS.some((p) => state.actuals[p].slice(Math.max(0, nowIdx - 4), nowIdx).some((v) => v == null)) && nowIdx > 0 ? "actuals missing — key them in the supply detail rows" : "actuals keyed" },
  ...
  const saveDeployment = (raw) => update((s) => {
    const d = { ...raw };
    if (d.stage === "soft") { if (!d.expiresAt) d.expiresAt = iso(addDays(today, s.params.holdDays)); } else delete d.expiresAt;
